...
func (m *Model) handleMergePRKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		if m.MergePanel == MergePanelCommit {
			...
			// Shortcut: Execute merge directly
			return m, m.mergePR()
		}
	}
...
func (m *Model) mergePR() tea.Cmd {
	return func() tea.Msg {
		...
		// Check for required approvals using PR's reviewDecision
		if len(m.PRs) > 0 && m.SelectedPR < len(m.PRs) {
...
// handleRefresh refreshes all data for the current view.
func (m *Model) handleRefresh() (tea.Model, tea.Cmd) {
	cmds := []tea.Cmd{m.fetchPRs(), m.fetchCurrentBranch(), m.fetchBranches()}
	switch m.ViewState {
	case ViewDashboard, ViewMergePR:
		if len(m.PRs) > 0 && m.SelectedPR < len(m.PRs) {
			cmds = append(cmds, m.fetchPRReviews(m.PRs[m.SelectedPR].Number))
		}
	case ViewBranchList:
		// Branch list needs fresh branches
	}
	return m, tea.Batch(cmds...)
}
