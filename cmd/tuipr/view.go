...
func (m *Model) dashboardFooter() string {
	...
	keys := "[j/k] Nav  [r] Refresh  [c] Create  [m] Merge  [q] Quit"
...
func (m *Model) createPRFooter() string {
	...
	keys := "[r] Refresh  [j/k] Nav  [1-3] Panel  [Enter] Select  [i] Edit  [Ctrl+s] Submit"
	if m.CreateMode == "insert" {
		keys = "[r] Refresh  [Esc] Normal  [Tab] Panel  [Ctrl+s] Submit"
	}
...
func (m *Model) renderMergeChecklistPanel() string {
	...
	// Use PR's reviewDecision for status
	reviewsOK := true
	reviewsLabel := "Approved"
	if len(m.PRs) > 0 && m.SelectedPR < len(m.PRs) {
		reviewDecision := m.PRs[m.SelectedPR].ReviewDecision
		...
		case "CHANGES_REQUESTED":
			reviewsOK = false
			reviewsLabel = "Changes Requested"
		case "REVIEW_REQUIRED":
			reviewsOK = false
			reviewsLabel = "Review Required"
		default:
			reviewsOK = false
			reviewsLabel = "No Reviews"
		}
	}
	items = append(items, infoItem(fmt.Sprintf("Reviews: %s", reviewsLabel), reviewsOK))
...
func (m *Model) mergePRFooter() string {
	...
	keys := "[r] Refresh  [j/k] Nav  [Space] Select  [Enter] Merge  [Esc] Back"
	return fmt.Sprintf("%s\n%s %s %s %s %s\n%s", sep, mode, p1, p2, p3, p4, keys)
}
...
func (m *Model) branchListView() string {
	var out strings.Builder
	out.WriteString(sep() + "\n")
	out.WriteString(centerText("SELECT TARGET BRANCH", 50) + "\n")
	out.WriteString(sep() + "\n\n")
